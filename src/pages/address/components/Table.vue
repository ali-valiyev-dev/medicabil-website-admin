<template>
  ...
  >
    ...
    <template #body-cell-header="props">
      <q-td :props="props">
        <div class="text-body2" :class="props.row.header ? 'text-secondary' : 'text-red-5'">
          {{ props.row.header ? 'Aktif' : 'Devre dışı' }}
        </div>
      </q-td>
    </template>

    <template #body-cell-footer="props">
      <q-td :props="props">
        <div class="text-body2" :class="props.row.footer ? 'text-secondary' : 'text-red-5'">
          {{ props.row.footer ? 'Aktif' : 'Devre dışı' }}
        </div>
      </q-td>
    </template>
...
import { useAddressStore } from 'src/stores/dialogs/addressStore'
import { storeToRefs } from 'pinia'
import { EditDialog, DeleteDialog } from '../components'

const { form } = storeToRefs(useFilterStore())
const { nextOrder, showEditDialog, showDeleteDialog } = storeToRefs(useAddressStore())
...
const columns = ref([
  ...
  { name: 'header', label: 'Site Üst Kısım', align: 'center', field: 'header' },
  { name: 'footer', label: 'Site Alt Kısım', align: 'center', field: 'footer' },
