...
const { showEditDialog } = storeToRefs(useReferenceStore())
...
onMounted(async () => {
  ...
  if (form.value.date) {
    let parsedDate = parse(form.value.date, 'dd.MM.yyyy', new Date())

    if (isNaN(parsedDate)) {
      parsedDate = new Date(form.value.date)
    }
    if (!isNaN(parsedDate)) {
      form.value.date = format(parsedDate, 'dd.MM.yyyy')
    } else {
      form.value.date = ''
    }
  }
